...
import org.burgerbytes.userinterface.Equipment.PostItem
...
class DatePostLoader : Loader<List<org.burgerbytes.userinterface.History.PostItem>> {

    override suspend fun loadPosts(db: FirebaseFirestore): List<org.burgerbytes.userinterface.History.PostItem> {
        val firebaseAuth = FirebaseAuth.getInstance()
        val currentUser = firebaseAuth.currentUser

        var posts = MutableStateFlow<List<org.burgerbytes.userinterface.History.PostItem>>(emptyList())
        // call to get posts
        db.collection("posts").orderBy("date", Query.Direction.DESCENDING).get().addOnSuccessListener { querySnapshot ->
            val postsList = querySnapshot.map { document ->
                org.burgerbytes.userinterface.History.PostItem(
